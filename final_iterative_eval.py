...
ITERATIVE_HORIZON = 12       # autoregressively roll this many steps
...
# ── output ──────────────────────────────────────────────────
OUT_DIR = TSLIB_DIR / f"iterative_results_H{TRAINED_HORIZON}_iter{ITERATIVE_HORIZON}"
...
def model_step(model: nn.Module,
    ...
    """One forward pass; returns the FIRST predicted step as (C,) numpy array."""
    C = x.shape[-1]
    dec_inp = torch.cat(
        [y_hist, torch.zeros(1, trained_horizon, C, device=device)], dim=1
    )  # (1, label_len + trained_horizon, C)

    out = model(x, x_mark, dec_inp, y_mark)  # (1, trained_horizon or more, C)
    return out[:, 0, :].cpu().numpy()[0]      # (C,)  — always take first output step
...
def run_iterative(model: nn.Module, ds, device: torch.device,
    ...
    """
    data_x     = ds.data_x      # (N_rows, C)
    data_y     = ds.data_y      # (N_rows, C)
    data_stamp = ds.data_stamp  # (N_rows, D)
    index_map  = ds.index_map   # valid window starts for test split
    ...
    for wi in range(0, n_windows, STRIDE):
        ...
        # ── initial context ──────────────────────────────────
        x_np     = data_x[s_begin : s_begin + SEQ_LEN]             # (L, C)
        sx_np    = data_stamp[s_begin : s_begin + SEQ_LEN]         # (L, D)
        ...
        if fut_e > len(data_y):
            break
        true_fut = data_y[fut_s : fut_e]                           # (H, C)
        ...
        for step in range(iterative_horizon):
            abs_row = fut_s + step
            # future time stamps (trained_horizon steps worth)
            fut_stamp_rows = []
            ...
            y_mark_np = np.concatenate([roll_syl, fut_stamp], axis=0)  # (label_len+trained_horizon, D)
            ...
            pred = model_step(model, x_t, sx_t, yh_t, ym_t, trained_horizon, device)
            # pred: (C,)

            step_preds.append(pred)

            # ── roll window forward by 1 ─────────────────────
            next_stamp = data_stamp[abs_row] if abs_row < len(data_stamp) else data_stamp[-1]
            roll_x   = np.concatenate([roll_x[1:],   pred[None, :]],       axis=0)
            roll_sx  = np.concatenate([roll_sx[1:],  next_stamp[None, :]], axis=0)
            roll_yh  = np.concatenate([roll_yh[1:],  pred[None, :]],       axis=0)
            roll_syl = np.concatenate([roll_syl[1:], next_stamp[None, :]], axis=0)

        all_preds.append(np.stack(step_preds, axis=0))   # (H, C)
    ...
    preds = np.stack(all_preds, axis=0)   # (N, H, C)
