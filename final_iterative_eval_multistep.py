"""
final_iterative_eval_multistep.py
---------------------------------
...
TRAINED_HORIZON = 3          # pred_len used during training / optuna sweep
...
# ── output ──────────────────────────────────────────────────
OUT_DIR = TSLIB_DIR / f"iterative_results_H{TRAINED_HORIZON}_iter{ITERATIVE_HORIZON}_with_inputs"
...
def run_one_model(model_name: str, ds, device: torch.device) -> dict:
    ...
    try:
        ...
        preds, trues, inputs = run_iterative(model, ds, device, TRAINED_HORIZON, ITERATIVE_HORIZON)
